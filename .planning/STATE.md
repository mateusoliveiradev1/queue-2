...
- [Phase 06-roleta-e-economia]: Focused Phase 6 tests now keep every ROUL requirement and D-01 through D-32 decision searchable. — This makes coverage gaps visible before the root gate composes the suites.
- [Phase 06-roleta-e-economia]: Roulette economy evidence is generated by a deterministic seeded simulation. — Base/boosted rarity, pity, cooldown, boost cost/refund/cap and weekend multiplier evidence can be regenerated with `node scripts/roulette-economy-simulation.mjs`.
- [Phase 06-roleta-e-economia]: The root Phase 6 gate writes setup, performance and security evidence artifacts. — Local deterministic checks, DB evidence and external blockers are consolidated without logging secret values.
- [Phase 06-roleta-e-economia]: Phase 6 performance review now has roulette-specific query targets. — State read, eligible pool, active round lookup, history, lock/discard, boost ledger and dashboard handoff are reviewed with matching hot-path indexes.
- [Phase 06-roleta-e-economia]: The final Phase 6 gate exits zero when local checks pass and missing external evidence is represented as BLOCKED. — Local implementation completion stays machine-checkable while release evidence remains honest.
...
- Phase 06 Plan 09 DB migration/RLS/concurrency evidence is blocked until TEST_DATABASE_URL points at an isolated Postgres/Neon test database.
- Phase 06 Plan 09 browser/accessibility evidence is blocked until E2E_BASE_URL, ready duo credentials, partner credentials, other-duo credentials and E2E_PHASE6_ELIGIBLE_SLUGS are configured.
- Phase 06 Plan 10 migration evidence is blocked until DATABASE_URL is configured for Drizzle generate/migrate in the gate environment.
- Phase 06 Plan 10 browser/accessibility gate evidence is blocked until E2E_PHASE6_ELIGIBLE_SLUGS is configured for the ready duo.
- Phase 06 final gate migration evidence remains blocked until DATABASE_URL is configured for Drizzle generate/migrate in the gate environment.
- Phase 06 final browser/accessibility evidence remains blocked until E2E_PHASE6_ELIGIBLE_SLUGS is configured for the ready duo.
